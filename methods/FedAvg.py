...
from entities.FedAvg import FedAvgClient, FedAvgServer
...
if __name__ == "__main__":
    config_path = PROJECT_ROOT / "configs" / "FedAvg.yaml"
    ...
    milestones = {
        int(value)
        for value in args.get("lr_decay_milestones", [])
    }
    ...
    for round_index in range(rounds):
        # Preserve the original zero-based milestone behavior.
        if round_index in milestones:
            args["lr"] *= float(args.get("lr_decay", 1.0))
        ...
        # FedAvg has one global model, so its global accuracy can be
        # evaluated directly on the server model.
        ...
        # Broadcast the same complete global model before client-level
        # Local and Missing evaluation.
