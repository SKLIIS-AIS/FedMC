...
class FedCCFAClient(Client):
    ...
    def update_label_distribution(self):
        self.label_distribution = torch.zeros(
            self.args["num_classes"], dtype=torch.long
        )
        ...
        # Dirichlet partitioning with a very small alpha may produce clients
        # with no training samples. Such clients have no label distribution
        # to estimate and must not trigger a division by zero.
        if len(self.train_set.targets) == 0:
            self.label_distribution = self.label_distribution.numpy()
            return
        ...
        self.class_mask = (self.label_distribution > 0).long()
        ...
        entropy = -np.sum(valid_prob * np.log(valid_prob))
    ...
    def train_with_protos(self, _round):
        ...
        with torch.no_grad():
            ...
            for label, proto in self.local_protos.items():
                self.global_protos[label] = proto.clone()
...
class FedCCFAServer(Server):
    ...
    def send_rep_params(self, clients):
        rep_keys = [
            ...
            if key not in self.clf_keys
        ]
        rep_state = {
            key: self.model.state_dict()[key].detach().clone()
            for key in rep_keys
        }
        for client in clients:
            client.set_rep_params(rep_state)
    ...
    def aggregate_rep(self, clients):
        ...
        float_keys = [
            key for key in rep_keys
            if self.model.state_dict()[key].is_floating_point()
        ]
        new_state = {
            key: torch.zeros_like(self.model.state_dict()[key])
        ...
        for client in clients:
            ...
            for key in float_keys:
                new_state[key] += client_size * client_state[key]

        state = self.model.state_dict()
        for key in float_keys:
            state[key] = new_state[key] / max(total_size, 1.0)
        self.model.load_state_dict(state)
    ...
    def aggregate_label_protos(self, label, clients):
        ...
        :param clients: Clients within the same group.
        """
        clients = [client for client in clients if label in client.local_protos]
        if not clients:
            return None
        if self.args["weights"] == "uniform":
